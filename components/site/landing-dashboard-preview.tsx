...
import {
  ...
  landingPreviewKpis,
  landingPreviewWeeklyRevenueEur,
  landingPreviewWeeklySessions,
} from "@/lib/landing-preview-metrics";
...
export function LandingDashboardPreview({
  ...
  const p = idPrefix;
  return (
    <div className="relative min-w-0 overflow-hidden rounded-2xl border border-border/80 bg-card/90 shadow-xl shadow-black/10 ring-1 ring-black/[0.04] backdrop-blur-md">
      ...
      <div className="relative min-w-0 p-3 sm:p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <span
            className="rounded-full border border-border/60 bg-muted/60 px-2.5 py-0.5 text-[10px] font-semibold text-muted-foreground"
            title={LANDING_PREVIEW_CAPTION}
          >
            Illustrative only
          </span>
        </div>
        <p className="mb-3 text-[11px] leading-snug text-muted-foreground/90">
          {LANDING_PREVIEW_CAPTION}
        </p>
        <div className="grid min-w-0 gap-3 sm:grid-cols-3">
          {landingPreviewKpis.map((k) => (
            <div
              key={k.label}
              className="rounded-2xl border border-border/70 bg-background/80 p-3 shadow-sm backdrop-blur-sm"
            >
              ...
              <div className="mt-2 flex justify-end opacity-90">
                <Sparkline
                  values={k.series}
                  color={k.color}
                  gradientId={`${p}-spark-${k.sparkId}`}
                  baselineZero={k.sparkId !== "retention"}
                />
              </div>
            </div>
          ))}
